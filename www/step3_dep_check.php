		...
		<table align='center' border='2'  class="fancytable">
			...
			<?php
				...
				while($unit=mysqli_fetch_assoc($unit_sql)){
					...
					//人數
					$check_name_sql1=mysqli_query($link,"select * from `master_list` as a,`reservation` as b where a.`mas_id`=b.`mas_id` and b.`unit_id`='$unit[unit_id]' and `res_status` != '1' group by `account_id`");//找人數(查帳號)
