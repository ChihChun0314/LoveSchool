		...
		<div class="w3-padding-large" id="main">
		  ...
		  <div class="w3-padding-64 w3-content w3-text-grey" id="contact">
			...
			<?php
				...
				//-----------------------------------------------------------------------------判斷時間區間
				/*$stime_sql=mysqli_query($link,"SELECT * FROM `master_list` AS a INNER JOIN `reservation` AS b ON a.`mas_id` = b.`mas_id` INNER JOIN `service_hours` AS c ON  b.`res_id` = c.`res_id` where c.`ser_day` = '$_POST[day]' and a.`account_id` = '$_POST[account_id]'");
				//echo "SELECT * FROM `master_list` AS a INNER JOIN `reservation` AS b ON a.`mas_id` = b.`mas_id` INNER JOIN `service_hours` AS c ON  b.`res_id` = c.`res_id` where c.`ser_day` = '$_POST[day]' and a.`account_id` = '$_POST[account_id]'";
				while($stime=mysqli_fetch_assoc($stime_sql)){
					echo "$stime[res_id] </br>";
					echo "$stime[start_time] </br>";
					echo "$stime[end_time] </br>";
					if ($str_time_hr < $stime['start_time'] && $str_time_hr > $stime['end_time']){
						echo "00000999988887777";
					}else{
						echo "幹 失敗的";
					}
				}*/
